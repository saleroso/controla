...
#Función salir
function salir() {
echo -e $red"Eliminando archivos creados..."$white
rm lista.txt 2> /dev/null 
rm listado.txt 2> /dev/null 
rm -r 4nonimizer 2> /dev/null 
sleep 1
echo -e $red"Parando servicios..."$white
gnome-terminal -e "4nonimizer stop" &> /dev/null
ps aux | grep "./program.sh" | cut -d' ' -f6 | while read mataproceso
		do
			kill -9 "$mataproceso" 2> /dev/null
		done 
echo -e "[*] Gracias por utilizar CONTROLA "
exit
}
...
cuenta="0"
touch l-$nombrearchivo.txt 2> /dev/null
while [ "$cuenta" -le 50 ]
do
                lynx "https://www.google.es/search?q=$dork&start=$cuenta" -dump -listonly | grep 'url?q=' | cut -d ' ' -f4 | sed 's/https:\/\/www.google.es\/url?q=//' | sed 's/\(&sa=\).*//' | cut -f2 -d"=" | limpieza | sort | uniq >> l-$nombrearchivo.txt
                cuenta=$(( $cuenta +10 ))
...
IPPUBLICA=$(curl ifconfig.me 2> /dev/null)
clear
banner
menulista="7"
echo -e $cyan" "
...
do
echo -e $cyan"1) Ocultar IP (IP actual $IPPUBLICA)"
...
function downfich() {
banner
listaurl="`cat l-$nombrearchivo.txt | cut -f1 -d" "` Volver"
total="`wc -l l-$nombrearchivo.txt | cut -f1 -d" "`"
valor="`cat l-$nombrearchivo.txt | wc -l`"
echo ""
...
2) menuwpscan ;;
3) wpscan --update &> /dev/null ; echo "" ; echo -e "WPscan ha sido actualizado! " ; sleep 2 ;echo "" ; menuwpscan ;;
4) wpscan --help ;;
5) externo ;;
